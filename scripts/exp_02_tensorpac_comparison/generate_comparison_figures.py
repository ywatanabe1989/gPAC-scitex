...
# File: ./scripts/exp_02_tensorpac_comparison/generate_comparison_figures.py
# ----------------------------------------
import os
...
def figure_3_performance_benchmark(
    ...
    if not baseline.empty:
        memory_data = {
            ...
            "GPU (MB)": [baseline["gpac_memory_gpu"].mean() / (1024*1024), 0]
        }
    ...
    # PLOT 3-7: Parameter sweeps (batch_size, n_chs, t_sec, pha_n_bands, amp_n_bands)
    sweep_params = [
        ("batch_size", gs[1, 0], "Batch Size"),
        ("n_chs", gs[1, 1], "Number of Channels"),
        ("t_sec", gs[1, 2], "Signal Duration"),
        ("pha_n_bands", gs[2, 0], "Number of Phase Bands"),
        ("amp_n_bands", gs[2, 1], "Number of Amplitude Bands"),
    ]

    for param, gs_pos, label in sweep_params:
        ax = plt.subplot(gs_pos)
        sweep_data = benchmark_df[benchmark_df["param_sweep"] == param] if "param_sweep" in benchmark_df.columns else pd.DataFrame()
        ...
        if not sweep_data.empty:
            ...
            ax.set_title(f"Computation Time vs {label}")
            ax.set_xlabel(label)
            ax.set_ylabel("Time (seconds)")
            ax.legend(["gPAC", "Tensorpac"])
            ax.grid(True, alpha=0.3)
        else:
            values = benchmark_df[param].unique() if param in benchmark_df.columns else []
            if len(values) > 1:
                by_param = benchmark_df.groupby(param).agg({
                    "gpac_calc_time": "mean",
                    "tensorpac_calc_time": "mean"
                })
                by_param.plot(marker="o", ax=ax)
                ax.set_title(f"Computation Time vs {label}")
                ax.set_xlabel(label)
                ax.set_ylabel("Time (seconds)")
                ax.legend(["gPAC", "Tensorpac"])
                ...
                ax.text(0.5, 0.5, f"No {param} sweep data", ha='center', va='center', fontsize=12)
                ax.set_title(f"Computation Time vs {label}")
    ...
    for param in ["batch_size", "n_chs", "t_sec", "pha_n_bands", "amp_n_bands"]:
        if "param_sweep" in benchmark_df.columns and any(benchmark_df["param_sweep"] == param):
        ...
        elif param in benchmark_df.columns and len(benchmark_df[param].unique()) > 1:
    ...
    if params_list:
        ...
        colors = plt.cm.viridis(np.linspace(0, 1, len(params_list)))
        bars = ax8.bar([param_labels.get(p, p) for p in params_list], speedups, color=colors, alpha=0.7)
