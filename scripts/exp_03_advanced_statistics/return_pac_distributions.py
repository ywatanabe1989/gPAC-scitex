...
# File: /ssh:sp:/home/ywatanabe/proj/gPAC/scripts/exp_03_advanced_statistics/return_pac_distributions.py
# ----------------------------------------
import os
__FILE__ = (
    "./scripts/exp_03_advanced_statistics/return_pac_distributions.py"
)
...
class PACDistributionAnalyzer:
    ...
    def calculate_thresholds(self, distributions: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Find largest p-value that satisfies p ≤ (i/m)α
...
def run_example(output_dir: str = './results') -> None:
    ...
    if len(result) == 4:
        ...
        # Save numerical results using mngs framework
        mngs.io.save(
            results_data,
        ...
        fig = analyzer.visualize_distributions(
            ...
            pha_freqs=pha_freqs,
            amp_freqs=amp_freqs,
            save_path=None  # Don't save directly, use mngs.io.save instead
        )
        
        # Save figure using mngs framework
        mngs.io.save(
            fig,
            "./results/exp_03/distribution_examples.png",
            symlink_from_cwd=True
        ...
        print("Analysis complete. Results saved using mngs framework.")
    else:
        print("Error: Distribution was not returned. Check n_perm and return_dist parameters.")


def run_main(args: Optional[argparse.Namespace] = None) -> int:
    """
